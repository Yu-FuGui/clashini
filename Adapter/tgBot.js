 ...
 */
const handleCallbackQuery = require('/bncr/BncrData/plugins/YU/mod/tgBot内联键盘'); // 引入新模块
/* 配置构造器 */
...
module.exports = async () => {
  ...
  tg.reply = async function (replyInfo, send = '') {
      try {
          ...
          if (replyInfo.type === 'text') {
              send = await tgBot.sendMessage(sendId, replyInfo.msg, replyInfo.options || {});
  ...
  tgBot.on('callback_query', async callbackQuery => {
      ...
      try {
          // 调用外部模块处理回调查询
          await handleCallbackQuery(data, callbackQuery, tgBot);
      } catch (e) {
          console.log('处理回调查询时发生错误:', e);
      }
   });
